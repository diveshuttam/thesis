...
xt = x
yt = a(np.array(x)+1)-a(np.array(x))
at = interpolate.interp1d(xt,yt,'linear',fill_value='extrapolate')
xct=np.linspace(x[0],x[-1],n)
targets = yct = a(xct+1.0) -a(xct)

# our scheme
sampling_scheme1 = sampling_schemes.periodic 
x1,y1 = map(list,sampling_scheme1(x,y,a,27))
x1t = x1
y1t = a(np.array(x1)+1)-a(np.array(x1))
print(len(x1),len(y1))
...
x2,y2 = map(list,sampling_scheme2(xt,yt,at))
