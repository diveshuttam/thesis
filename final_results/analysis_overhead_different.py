...
for parameter in parameters_uti.parameters_array:
    # uti
    cemon_x, cemon_y = get_overhead_cemon(parameter_5)
    curvature_x, curvature_y, momon_x, momon_y = get_overhead_curvature_and_momon(parameter, parameter_3) # from polling as single flows
    ...
    plt.legend()
    plt.savefig(f"overhead_graphs/different_tmax/without_cemon/without_cemon_overhead_"+"_".join(map(lambda x: str(float(x)),parameter))+".png")
