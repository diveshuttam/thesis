...
def get_overhead_curvature_and_momon(parameter):
...
for parameter in parameters_uti.parameters_array:
    ...
    curvature_x, curvature_y, momon_x, momon_y = get_overhead_curvature_and_momon(parameter) # from polling as single flows
    print(f"using paramter {parameter}")
    plt.figure()
    plt.plot(curvature_x, curvature_y, label="curvature")
    plt.plot(momon_x, momon_y, label="momon")
    plt.plot(cemon_x,cemon_y, label="cemon")
    plt.xlabel("Time (s)")
    plt.ylabel("Message Overhead")
    plt.title(f"Message Overhead of various schemes\n Paramters {parameter}")
    plt.legend()
    plt.savefig(f"overhead_graphs/both_same_tmax/all/all_overhead_"+"_".join(map(lambda x: str(float(x)),parameter))+".png")
